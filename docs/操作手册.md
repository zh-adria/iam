...
|------|--------------------|--------------------|
| 开发模式（MySQL + vite） | `./scripts/dev.sh` | `.\scripts\dev.ps1` |
...
- 仅拉起 redis（docker），**不启动 mysql** — 后端用 MySQL 数据库
...
| MySQL 数据 | `~/iam-dev/iam.*`（Linux/Mac）或 `%USERPROFILE%\iam-dev\iam.*`（Windows） | dev profile 文件库（AUTO_SERVER 共享） |
...
**Q：后端报数据库连接失败？**
A：检查 MySQL 容器是否正常启动（`docker compose up -d mysql`）。
