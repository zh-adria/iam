...
│   │       ├── infrastructure/
│   │       │   ├── entity/        # 17 个 JPA 实体
│   │       │   ├── repository/    # 17 个 Spring Data repo
...
17 张数据库表（Liquibase `db/changelog/master.xml` 管理）：
...
| `auth_audit_log` | id, principal, action, ip, occurred_at, hash_chain_prev | 哈希链防篡改 |
...
| `AuthController.loginOptions` / LoginView | `iam.login.methods` / `iam.login.social-providers` | STRING |
